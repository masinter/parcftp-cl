...
\i{Ascii}.  An \i{arg}, any \term{object}, 
is printed without escape characters
(as by \funref{princ}).  If \i{arg} is a \term{string}, 
its \term{characters}
will be output verbatim.
If \i{arg} is \nil\ it will be printed as \nil;
the \term{colon} modifier (\f{~:A}) will cause an \i{arg} of \nil\ to be printed as \empty,
but if \i{arg} is a composite structure, such as a \term{list} or \term{vector},
...
therefore suitable for input to \funref{read}.  {\tt ~S} accepts
all the arguments and modifiers that {\tt ~A} does.

\issue{FORMAT-PRETTY-PRINT:YES}
{\tt ~S} binds \varref{*print-escape*} to \t.
\endissue{FORMAT-PRETTY-PRINT:YES}
...
\i{Decimal}.
An \i{arg}, which should be an \term{integer}, 
...
%% 22.3.2 23
If \i{arg} is not an \term{integer}, it is printed in {\tt ~A} format and decimal base.
...
\i{Plural}.
If \i{arg} is not \funref{eql} 
to the integer \f{1}, a lowercase \f{s} is
printed; if \i{arg} is \funref{eql} to \f{1}, nothing is printed.  
...
\i{Character}.  The next \i{arg} should be a \term{character}; 
it is printed
...
%% 22.3.2 40
{\tt ~:C} is the same as \f{~C} for \term{printing} \term{characters},
...
\issue{FORMAT-PRETTY-PRINT:YES}
{\tt ~@C}  binds \varref{*print-escape*} to \t.
\endissue{FORMAT-PRETTY-PRINT:YES}
...
The next \i{arg} is printed as a \term{float}.
...
%% 22.3.2 52
If \i{arg} is a \term{complex} number or some non-numeric
\term{object}, 
...
The next \i{arg} is printed as a \term{float} in exponential notation.
...
\i{General floating-point}.  The next \i{arg} is printed as a \term{float} 
...
\i{Dollars floating-point}.  The next \i{arg} is printed as a \term{float}
...
the {\tt ~\$} directive, and where \i{q}=\i{d}+\i{n}\minussign 1,
...
\issue{FORMAT-PRETTY-PRINT:YES}
{\tt ~\$} binds \varref{*print-escape*} to \term{false}
\issue{PRINC-READABLY:X3J13-DEC-91}
 and \varref{*print-readably*} to \term{false}.
\endissue{PRINC-READABLY:X3J13-DEC-91}
...
\i{Tilde}.
This outputs a \term{tilde}.  {\tt ~\i{n}~} outputs \i{n} tildes.
...
\i{Indirection}.
The next \i{arg} must be a \term{format control}, and the one after it a \term{list};
...
The \i{arg} must be a \term{string}; 
...
%% 22.3.2 117   
{\tt ~:(} capitalizes all words, as if by \funref{string-capitalize}.
...
%% 22.3.2 121                                                 
\itemitem{\tt ~[\i{str0}~;\i{str1}~;\i{...}~;\i{strn}~]}
...
%% 22.3.2 123
{\tt ~:[\param{alternative}~;\param{consequent}~]} 
selects the \param{alternative} control string if \i{arg} is \term{false},
...
If the \i{arg} is \term{false}, then the argument is used up,
...
The \term{string} \i{str} is used repeatedly as the control string.
...
used as a \term{list} of arguments to \i{str}.
Example:
...
%% 22.3.2 136                           
\itemitem{\tt ~\i{mincol},\i{colinc},\i{minpad},\i{padchar}<\i{str}~>}
...
%% 22.3.2 139
Note that \i{str} may include \funref{format} directives.
...
The first four arguments are:
     ...
     a \term{boolean} that is \term{true} if the \term{colon} modifier was used,
 and a \term{boolean} that is \term{true} if the \term{at-sign} modifier was used.
...
if \term{colon} or \term{at-sign} modifiers are given to a directive in a combination not
