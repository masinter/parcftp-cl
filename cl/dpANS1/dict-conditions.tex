...
\param{place}---a \term{generalized reference} acceptable to \macref{setf};
evaluated if an error is signaled.
...
The \param{places} should be \term{generalized references} on which \param{test-form} depends, 
...
It is later \term{evaluated} as a \term{generalized reference} 
...
\param{expression} returns normally and a \i{no-error-clause}
 ...
 \f{(lambda \param{lambda-list} \star\curly{\param{form}})}
   from the \i{no-error-clause}, and the \term{values} of that function call are
   returned by \macref{handler-bind}.
...
\param{parent-type}---a \term{non-nil} \term{symbols} 
  naming \term{condition} \term{types}.
  If no \param{parent-types} are supplied,
  \param{parent-types} defaults to \f{(condition)}.
...
\funref{abort} transfers control to the most recently established
\term{applicable restart} named \misc{abort}.
...
Presence of a restart named \misc{abort}.

\label Exceptional Situations::

If no \misc{abort} restart exists,
\funref{abort} signals an error \oftype{control-error}.
...
%%% ========== CONTINUE
\begincom{continue}\ftype{Function}
...
the most recently established \term{applicable restart} named \misc{continue}.
...
A transfer of control to \therestart{continue} occurs if such a \term{restart} 
is currently exists.

\label Affected By::

Presence of a restart named \misc{continue}.

\label Exceptional Situations:\None.

\label See Also::

\funref{invoke-restart}, \macref{assert}, \funref{cerror}
...
%%% ========== MUFFLE-WARNING
\begincom{muffle-warning}\ftype{Function}
...
Transfers control to 
the most recently established \term{applicable restart} named \misc{muffle-warning}.
If no such \term{restart} exists, an error \oftype{control-error} is signaled.
...
A dynamically active restart named \misc{muffle-warning}.

\label Exceptional Situations::

If no \misc{muffle-warning} \term{restart} is active,
\thefunction{muffle-warning} signals an error \oftype{control-error}. 
...
%%% ========== STORE-VALUE
\begincom{store-value}\ftype{Function}
...
\funref{store-value} transfers control and one \term{value} to the
most recently established \term{applicable 8restart} named \misc{store-value}.
If no such \term{restart} exists, \funref{store-value} returns \nil.
...
\OUT Debug> \IN{(store-value 12)}
\EV 12
 (careful-symbol-value 'a) \EV 12
\endcode
...
Presence of a \misc{store-value} restart.

\label Exceptional Situations:\None.

\label See Also::

\funref{use-value}, \macref{check-type}, \macref{ccase},
\macref{ctypecase}, {\secref\Restarts}
...
%%% ========== USE-VALUE
\begincom{use-value}\ftype{Function}
...
Transfers control and one \term{value} to 
the most recently established \term{applicable restart} named \misc{use-value}.
If no such \term{restart} exists, \funref{use-value} returns \nil.
...
Presence of \misc{use-value} restart.
