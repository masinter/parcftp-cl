...
\auxbnf{print-function-option}{\paren{\kwd{print-function} \param{print-function-name}} |
...
%!!! Should this perhaps be "string designator"?
\param{conc-name}---a \term{symbol name designator}.
...
\param{print-function-name}---a \term{function name} or a \term{lambda expression}.

\param{slot-initform}---a \term{form}.

\param{slot-read-only-p}---a \term{boolean}.
...
used as \param{slot-initforms} for the keyword parameters of the
constructor function.
...
one \term{argument}.  The copier function creates a \term{fresh} 
...
\itemitem{\kwd{print-function}}

%% 19.5.0 21
This option can be used only if \kwd{type}
is not supplied.
The argument to \kwd{print-function} 
should be 
in a form acceptable to \specref{function}, and is
...
the structure to be printed, a \term{stream} to print to,
and an \term{integer} 
...
If \kwd{print-function} and \kwd{type}
are not supplied, or if \kwd{print-function}
is supplied with no argument,  then a default printing function is
provided for the structure that prints out all its slots
using \f{\#S} syntax.
If \kwd{print-function} and \kwd{type}
are not supplied, but a print function is inherited via the \kwd{include}
option, that print function is used to print the slots.

Supplying \kwd{print-function} to \macref{defstruct} is equivalent
to defining an appropriate \term{method} 
on the \funref{print-object} generic function. \Seesection\IntroToMethods.
\reviewer{Barmar: \funref{print-object} and \kwd{print-function} differ
regarding \varref{*print-level*}.}%!!!
...
\funref{write}, \funref{prin1}, \funref{princ}, or 
\funref{format}
...
\kwd{print-function} options. If a user-defined print function prints to a 
\term{stream} other than the one
that was supplied, then circularity detection starts over for that
\term{stream}. 
See \varref{*print-circle*}.
...
The \f{\#S} \term{reader macro} might or might recognize the newly defined 
...
%% 19.7.2 1
The argument to \kwd{print-function} should observe the values of
