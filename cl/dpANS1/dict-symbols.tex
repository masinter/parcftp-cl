...
\DefunWithValues symbolp {object} {boolean}
...
\DefunWithValues keywordp {object} {boolean}
...
\param{copy-properties}---a \term{boolean}.
  \Default{\term{false}}
...
\EV #<FUNCTION anonymous>
 (list (twice 3)
       ...
       (funcall (symbol-function 'twice) 3))
\EV ((3 3) (3 3) (3 3))
 (fboundp 'defun) \EV \term{implementation-dependent}
...
if one is found.  Otherwise \param{default} is returned.
...
\DefunWithValues remprop {symbol indicator} {boolean}
...
Removes an entry from the \term{property list} of \param{symbol},
returning \term{false} if no such property was found,
or \term{true} if a property was found.

%% 10.1.0 13
%!!! Sandra thinks this could be worded more tightly. The first sentence is nearly
%    redundant except that it specifies the test.
The property of \param{symbol} with an indicator \funref{eq} to
\param{indicator} is removed.  The property indicator and the 
corresponding value are removed in an undefined order by 
destructively splicing the property list.  

%!!! This issue impact needs to be expressed more perspicuously. -kmp 24-May-91
%    e.g., Barmar suggests
%       (remprop x y) is required to be equivalent to (remf (symbol-plist x) y).
...
\DefunWithValues boundp {symbol} {boolean}
