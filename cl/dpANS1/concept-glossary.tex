...
%\term{variable} removed since generalized reference implies it.
  1. \TransitiveVerb\ (a \term{generalized reference}, or \term{array})
     to \term{read}\meaning{1} or \term{write}\meaning{1} the \term{value} of
         the \term{generalized reference}
      or an \term{element} of the \term{array}.
  2. \Noun\ (of a \term{generalized reference})
     an attempt to \term{access}\meaning{1}
     the \term{value} of the \term{generalized reference}.
...
\gentry{actual adjustability} \Noun\ (of an \term{array})
  a \term{boolean} that is associated with the \term{array}, 
...
\gentry{boolean} \Noun\ 
  1. an \term{object} used as a truth value, where the symbol~\nil\ 
     represents \term{false} and all other \term{objects} represent \term{true}.
  2. one of the following \term{objects}: 
  	   the symbol~\t\   (representing \term{true}),
	or the symbol~\nil\ (representing \term{false}).

\gentry{boolean equivalent} \Noun\ (of an \term{object})
  any \term{object} which has the same truth value as \term{object}.
...
\gentry{coalesce} \TransitiveVerb\ (\term{constant objects} that are \term{similar})
  to consolidate the identity of those \term{objects},
  such that they become the \term{identical} \term{object}.
...
\gentry{compiler} \Noun\
  ...
  \Seefuns{compile} and \funref{compile-file}.
...
\gentry{constant form} \Noun\
  any \term{form} for which \term{evaluation} always \term{yields} the
  same \term{value} and which neither affects nor is affected by the
  \term{environment} in which it is \term{evaluated}.
...
\gentry{environment object} \Noun\
  an \term{object} representing a set of \term{lexical bindings},
     ...
     \gexample{\funref{macroexpand} takes an optional environment argument.}
     (The \term{object} \nil\ when used as an \term{environment}\meaning{2}
      \term{object} denotes the \term{null lexical environment};
...
\gentry{expressed adjustability} \Noun\ (of an \term{array})
  a \term{boolean} that is conceptually (but not necessarily actually)
...
\gentry{generalized reference} \Noun\
  ...
  \Seesection\GeneralizedReference.
...
\gentry{keyword/value pair} \Noun\ 
  ...
  respectively) of a \term{list} that is in \term{property list format}.
...
\gentry{leap seconds} \Noun\
  additional one-second intervals of time that are periodically inserted 
...
\gentry{list structure} \Noun\ (of a \term{list})
  ...
  actually contains one of its \term{sublists} as an \term{element}.
...
\gentry{namestring} \Noun\ 
  a \term{string} that conforms to \term{implementation-defined} conventions for 
  naming a \term{file}.
...
\gentry{nil} \Noun\ 
  the \term{object} that is at once
        ...
        the \term{empty list},
        the \term{boolean} representing \term{false},
...
\gentry{non-nil} \Noun, \Adjective\ 
  ...
  of the \term{object} as a \term{boolean}.  Referring to such an \term{object}
  as \term{non-nil} avoids this implication.
...
\gentry{package designator} \Noun\
  ...
  and that is one of:
      a \term{string} (denoting the \term{package} that has that \term{string}
                       as its \term{name} or as one of its \term{nicknames}),
      a \term{symbol} (denoting the \term{package} that has that \term{symbol}'s
                       \term{name} as its \term{name} or as one of its \term{nicknames}),
...
\gentry{place} \Noun\ 
  ...
  2. the conceptual location referred to by such a \term{generalized reference}.
...
\gentry{potential number} \Noun\ 
  ...
  \Seesection\PrintingPotentialNumbers\ and \secref\InterpOfTokens.
...
\gentry{predicate} \Noun\ 
  a \term{function} that returns a \term{boolean} as its first value.
...
\gentry{property list} \Noun\ 
  1.  a \term{list} containing an even number of \term{elements} that are
      ...
      or \term{keys}) and \term{values} (sometimes called 
      \term{properties}), and in which all \term{keys} 
      are \term{different} under \funref{eq}.
  2. (of a \term{symbol})
...
\gentry{read} \TransitiveVerb\ 
  1. (a \term{variable} or \term{slot})
     to obtain the \term{value} of the \term{variable} or \term{slot}.
...
\gentry{section} \Noun\
  a partitioning of output by a conditional newline on a \term{pretty printing stream}.
...
\gentry{setf expander} \Noun\ 
  ...
  of a \term{generalized reference}.
...
\gentry{setf expansion} \Noun\ 
  a set of five \term{expressions}\meaning{1} that, taken together, describe 
       how to store into a \term{generalized reference} 
   and which \term{subforms} of the macro call associated with the
       \term{generalized reference} are evaluated.
...
\issue{TAILP-NIL:T}
\gentry{sublist} \Noun\ (of a \term{list})
  ...
  the \term{list}.
  \gexample{The empty list is a sublist of every proper list.}
...
\gentry{supplied-p parameter} \Noun\
  a \term{parameter} which recieves its \term{boolean} value implicitly
  due to the presence or absence of an \term{argument} corresponding to
  another \term{parameter} (such as an \term{optional parameter} or a 
  \term{rest parameter}).  \Seesection\OrdinaryLambdaLists.
...
\gentry{t} \Noun\ 
  1. the canonical \term{boolean} representing true.
...
\gentry{write} \TransitiveVerb\ 
  1. (a \term{variable} or \term{slot})
     to change the \term{value} of the \term{variable} or \term{slot}.
