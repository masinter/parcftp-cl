...
\DefunWithValues fboundp {name} {boolean}
...
\code
 ...
 (fboundp 'with-open-file) \EV \term{implementation-dependent}
 (fboundp 'unwind-protect) \EV \term{implementation-dependent}
...
\EV \term{false}
 ...
 (fboundp 'my-macro) \EV \term{implementation-dependent}
...
\DefunWithValues functionp {object} {boolean}
...
\DefunWithValues compiled-function-p {object} {boolean}
...
%% 7.10.0 4
The order of execution of \specref{catch} follows:
...
\nil\ represents both \term{boolean} \term{false} and the \term{empty list}.
...
\param{x}---a \term{boolean} (\ie any \term{object}).
...
%% 6.4.0 3
\Predicate{x}{\term{false}}

% \term{Calling} \funref{not} to \param{x} when \param{x} 
% is a boolean value
% inverts \param{x}.
...
\code
 (not nil) \EV \term{true}
 (not '()) \EV \term{true}
 (not (integerp 'sss)) \EV \term{true}
 (not (integerp 1)) \EV \term{false}
 (not 3.7) \EV \term{false}
 (not 'apple) \EV \term{false}
\endcode
...
The canonical \term{boolean} representing true.  Although any \term{object} 
other than \nil\ is considered \term{true}, \t\ is generally used when
there is no special reason to prefer one such \term{object} over another.
...
\DefunWithValues eq {x y} {boolean}
...
\DefunWithValues eql {x y} {boolean}
...
\DefunWithValues equal {x y} {boolean}
...
\DefunWithValues equalp {x y} {boolean}
...
Returns a \term{function} that
  ...
  but returns only a single value:
   a \term{boolean} with the opposite truth value of that
...
\DefunWithValues every    {predicate {\rest} \plus{sequences}} {boolean}
\DefunWithValues some     {predicate {\rest} \plus{sequences}} {result}
\DefunWithValues notevery {predicate {\rest} \plus{sequences}} {boolean} 
\DefunWithValues notany   {predicate {\rest} \plus{sequences}} {boolean}
...
\param{keyplace}---a \term{form}; evaluated initially to produce a \param{test-key}.
   Possibly also used later as a \term{generalized reference} if no \param{keys} match.
...
\param{keyplace}---a \term{form}; evaluated initially to produce a \param{test-key}.
   Possibly also used later as a \term{generalized reference} if no \param{types} match.
...
with those used by \macref{setf}.
% added  --sjl 4 Mar 92
...
\DefspecWithValues progn {\star{\param{form}}} {\starparam{result}}
...
\param{name} to \term{read} and \term{write} a \term{generalized reference}.

The arguments to the new \term{macro} are a \term{generalized reference},
followed
...
is applied to the old contents of the \term{generalized reference}
and the \param{lambda-list} arguments to obtain the new value,
and the \term{generalized reference} is updated to contain the result.
...
\macref{setf} a \term{generalized reference}
...
%
% When \macref{setf} is given a \term{generalized reference} that is
...
% to be assigned to the \term{generalized reference}. \param{Update-fn} must return
...
than \param{access-fn} takes.  When \macref{setf} is given a \term{generalized reference}
...
to be stored into the \term{generalized reference}.
The \param{body} must
...
of the generalized reference are evaluated exactly once.
...
at compile time if the \param{access-fn} is used in a \term{generalized reference}
...
function that simultaneously stores into the \term{generalized reference} and 
...
a \term{generalized reference} that is a byte, such as \f{(ldb field reference)}.
\issue{SETF-METHOD-VS-SETF-METHOD:RENAME-OLD-TERMS}
\macref{define-setf-expander}
\endissue{SETF-METHOD-VS-SETF-METHOD:RENAME-OLD-TERMS}
...
updates a \term{generalized reference} that is referenced by \param{access-fn}.

When \macref{setf} is given a \term{generalized reference} that is
specified in terms of \param{access-fn} and a new value for the 
\param{place}, it is expanded into a form that performs
...
% corresponding \param{access-fn} arguments in the \term{generalized reference}.
% %% 7.2.0 73
% \param{lambda-list} specifies the arguments of \param{access-fn} by
% describing the \term{subforms} of the \term{generalized reference}.
% \issue{GET-SETF-METHOD-ENVIRONMENT:ADD-ARG}
% %% 8.1.0 12
% \keyref{environment}
...
that references the \term{generalized reference} by means of the given
\param{access-fn}.
\issue{DEFINING-MACROS-NON-TOP-LEVEL:ALLOW}
% Reworded garbled text.  --sjl 7 Mar 92
...
\endissue{DEFINING-MACROS-NON-TOP-LEVEL:ALLOW}
While \param{forms} are being executed, the variables in \param{lambda-list}
are bound to parts of the \term{generalized reference}.
\issue{FLET-IMPLICIT-BLOCK:YES}
\issue{DEFMACRO-BLOCK-SCOPE:EXCLUDES-BINDINGS}
...
% 	       of the \term{generalized reference} 
...
in \param{lambda-list} are bound to parts of the \term{generalized reference},
...
% that allow storing multiple values into a \term{generalized reference}. 
% 
...
% %a reference to a \term{generalized reference}.
...
\code
 (let ((var1 \param{place1})
   ...
   ...
   (setf \param{placen} \param{newvalue})
   var1)
\endcode
