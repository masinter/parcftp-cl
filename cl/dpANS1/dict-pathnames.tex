...
After the components supplied explicitly by \param{host}, \param{device}, 
\param{directory}, \param{name}, \param{type}, and \param{version} are filled in,
the merging rules used by \funref{merge-pathnames} are used to fill
in any missing components from the defaults supplied by \param{defaults}.
...
if and only its host component denotes a \term{logical host}.
...
\issue{PATHNAME-LOGICAL:ADD}
\funref{merge-pathnames}, \typeref{pathname}, \typeref{logical-pathname}, {\secref\FileSystemConcepts}
\endissue{PATHNAME-LOGICAL:ADD}
...
\DefunWithValues pathnamep {object} {boolean}
...
Implementations can define additional \term{functions} that operate on
\term{logical pathname} hosts, for example to specify additional translation
rules or options.
...
\issue{PATHNAME-LOGICAL:ADD}
\funref{truename}, \funref{merge-pathnames},
\typeref{pathname}, \typeref{logical-pathname}, {\secref\FileSystemConcepts}
\endissue{PATHNAME-LOGICAL:ADD}
...
\DefunWithValuesNewline parse-namestring
		        {thing {\opt} host defaults {\key} start end junk-allowed}
			{pathname, position}
...
\issue{PATHNAME-SYMBOL}
\issue{PATHNAME-LOGICAL:ADD}
\param{thing}---a \term{pathname designator}.
\endissue{PATHNAME-LOGICAL:ADD}
\endissue{PATHNAME-SYMBOL}

\issue{PATHNAME-LOGICAL:ADD}
\issue{PATHNAME-UNSPECIFIC-COMPONENT:NEW-TOKEN}
\param{host}---a \term{valid pathname host} or a \term{logical host}.
\endissue{PATHNAME-UNSPECIFIC-COMPONENT:NEW-TOKEN}
\endissue{PATHNAME-LOGICAL:ADD}
...
\param{junk-allowed}---a \term{boolean}.
  \Default{\term{false}}
...
% If \param{thing} is a \term{symbol}, the print
% name is used.
\endissue{PATHNAME-SYMBOL}

%% 23.1.2 11
\issue{PATHNAME-SYMBOL}
When \param{thing} is a \term{string} 
%%or \term{symbol}
\endissue{PATHNAME-SYMBOL}
\funref{parse-namestring}
parses a file name within \param{thing} in the
range delimited by \param{start} and \param{end}.
\param{start} is an index into \param{thing} when \param{thing} is
a \term{string}. 
\param{end} is an index into \param{thing} when \param{thing} is
a \term{string}. 
If \param{thing} is a \term{pathname} or a \term{stream},
no parsing is done, but \param{host} and the host component of 
\param{thing} are compared.
\issue{PATHNAME-STREAM}
If \param{thing} is a \term{pathname} (as returned by \funref{pathname})
it represents the name used to open the file. This may be, but is
not required to be, the actual name of the file. 
If \param{thing} is a \term{stream}, that \term{stream}
...
when \param{host} is logical or \param{defaults} is
a \term{logical pathname}.   
In this case the host portion of the 
\term{logical pathname} namestring and its following \term{colon} are optional.  
\param{host} is logical if it is supplied and came from
\funref{pathname-host} of a 
\term{logical pathname}.  Whether a host argument is logical
  if it is a \term{string} that is \term{string equal} to a 
\term{logical pathname} host name is
  \term{implementation-defined}.
If the host
  portion of the namestring and \param{host} are both present and do
...
If \param{host} is \nil\ then the host 
name is extracted from \param{defaults}
and used to determine the syntax convention.  

If \param{defaults} is not supplied,
its value is \varref{*default-pathname-defaults*}.

%% 23.1.2 12
If \param{junk-allowed} is \term{true},
then the first value returned is the \term{pathname} parsed 
or, if no syntactically correct \term{pathname} was seen, \term{false}.
%% 23.1.2 13
If \param{junk-allowed} is \term{false},
then \param{thing} is scanned.
The returned value is the \term{pathname} parsed.

%% 23.1.2 14
In either case, the second value is the index 
into \param{thing} of the delimiter
that terminated the parse, or the index beyond 
subthing if the
parse terminated at the end of subthing (as will always be the case if
\param{junk-allowed} is \term{false}).
%% 23.1.2 15    
If \param{thing} is not a \term{string} 
\issue{PATHNAME-SYMBOL}
% The following phrase will be deleted:
% or \term{symbol}, 
\endissue{PATHNAME-SYMBOL}
then \param{start} is returned as the second value.

%% 23.1.2 16
Parsing an empty \term{string} 
always succeeds, producing a \term{pathname} with
all components except the host equal to \nil.

%% 23.1.2 18
If \param{thing} contains an explicit host name and no explicit device name,
\funref{parse-namestring} may supply the
standard default device for that host as the device component
of the resulting \term{pathname}, depending on the implementation.

For the meaning of \kwd{unspecific}, \seesection\UnspecificComponent.
...
\DefunWithValues wild-pathname-p {pathname {\opt} field-key} {boolean}
...
\DefunWithValues pathname-match-p {pathname wildcard} {boolean}
...
\issue{PATHNAME-LOGICAL:ADD}
\funref{directory}, \typeref{pathname}, \typeref{logical-pathname}, {\secref\FileSystemConcepts}
\endissue{PATHNAME-LOGICAL:ADD}
...
\funref{logical-pathname}, \funref{logical-pathname-translations}, 
\typeref{logical-pathname}, {\secref\FileSystemConcepts}
...
\DefunWithValuesNewline merge-pathnames
		 	{pathname {\opt} defaults default-version}
			{merged-pathname}
...
%% 23.1.2 19
Constructs a \term{pathname} from \param{pathname} by filling in any
unsupplied components with the corresponding values from \param{defaults} 
and \param{default-version}. 

%% 23.1.2 22
Defaulting of pathname components is done by filling in components taken
from another \term{pathname}.  Unsupplied components of the output
\term{pathname} come from \param{pathname}, except that the type should
default not to the type of \param{pathname} but to the appropriate default type
...
if the host component of \param{defaults} matches the host component
of \param{pathname}, then the device is taken from the \param{defaults};
...
or type, each such component is copied from \param{defaults}.
If \param{pathname} does not specify a name, then the version, if not provided, will
come from \param{defaults}, just like the other components.  If
\param{pathname} does specify a name, then the version is not affected
by \param{defaults}. If this process leaves the
...
and type default but supply a version different from the one in \param{defaults}.
...
not required to be, the actual name of the file. 
\issue{PATHNAME-LOGICAL:ADD}
\funref{merge-pathnames} recognizes a 
\term{logical pathname} \term{namestring} when \param{defaults} is a 
\term{logical pathname}.  In this case the host portion of the 
\term{logical pathname} \term{namestring} and its following \term{colon} are optional.
\endissue{PATHNAME-LOGICAL:ADD}

\issue{PATHNAME-LOGICAL:ADD}
\funref{merge-pathnames} returns a 
\term{logical pathname} if and only if its first argument is a 
\term{logical pathname} or its first argument does not specify a
  host and the default is a \term{logical pathname}. 
\endissue{PATHNAME-LOGICAL:ADD}
...
\f{(pathname-directory \param{defaults})} is a \term{list}, then
...
If \f{(pathname-directory \param{defaults})} is not a \term{list} or
\f{(pathname-directory \param{pathname})} is not a \term{list} 
whose \term{car} is \kwd{relative}, the merged directory is
\f{(or (pathname-directory \param{pathname}) (pathname-directory \param{defaults}))}
\endissue{PATHNAME-SUBDIRECTORY-LIST:NEW-REPRESENTATION}
...
%% 23.1.2 25
The net effect of \funref{merge-pathnames} is that if just a name is supplied, 
then the host, device, directory, and type will come from \param{defaults}, 
but the version will come from \param{default-version}.  If nothing or
just a directory is supplied, the name, type, and version will come from
\param{defaults} together.
