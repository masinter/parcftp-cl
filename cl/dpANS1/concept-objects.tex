...
%%Barmar suggests deleting the next line "to...also" because plists don't allow duplicates.
%to a \term{property list} and also 
to the portion of an argument list
processed for \keyref{key} parameters.  As in those lists, if an initialization
...
returns a boolean value that indicates whether a specified \term{slot} has a
...
The elided code in the definition of \funref{make-instance} checks the
